...
const CommandBlock = ({
  ...
  useEffect(() => {
    ...
    // `monaco.languages.typescript` is deprecated in monaco-editor >=0.53 and
    // only present when the TypeScript worker is loaded, so access it
    // defensively rather than through the (now stubbed) static typings.
    const tsLanguage = (
