...
func (q *queueProcessor) EnqueueBatch(ctx context.Context, items []Item, ats []time.Time, opts EnqueueOpts) []error {
	if batchEnqueuer, ok := q.Producer.(BatchEnqueuer); ok {
		return batchEnqueuer.EnqueueBatch(ctx, items, ats, opts)
	}

	errs := make([]error, len(items))
	for idx := range items {
		errs[idx] = q.Producer.Enqueue(ctx, items[idx], ats[idx], opts)
	}
	return errs
}
