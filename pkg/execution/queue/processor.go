...
// EnqueueBatch delegates to the producer's batch enqueue implementation,
// satisfying the BatchEnqueuer optional interface on queueProcessor.
func (q *queueProcessor) EnqueueBatch(ctx context.Context, items []Item, ats []time.Time, opts EnqueueOpts) []error {
	if bp, ok := q.queueProducer.(BatchEnqueuer); ok {
		return bp.EnqueueBatch(ctx, items, ats, opts)
	}
	errs := make([]error, len(items))
	for i := range items {
		errs[i] = q.Enqueue(ctx, items[i], ats[i], opts)
	}
	return errs
}
