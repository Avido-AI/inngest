...
func (e *executor) buildBatchInvokeItems(ctx context.Context, i *runInstance, inputs []batchInvokeInput, edge queue.PayloadEdge, eventName string, lifecycleItem queue.Item, group OpcodeGroup) ([]batchInvokeItem, error) {
	...
	for _, input := range inputs {
		item, err := e.buildSingleInvokeItem(ctx, i, input, edge, eventName, lifecycleItem, now, handledAt)
		if err != nil {
			dropUnprocessedSpans(items, 0)
			return nil, err
		}
...
func (e *executor) buildSingleInvokeItem(ctx context.Context, i *runInstance, input batchInvokeInput, edge queue.PayloadEdge, eventName string, lifecycleItem queue.Item, now time.Time, handledAt time.Time) (batchInvokeItem, error) {
	gen := input.gen
	...
	if err := evt.Event.Meta.Sessions.Validate(); err != nil {
		return batchInvokeItem{}, execError{err: fmt.Errorf("invalid step.invoke sessions: %w", err), final: true}
	}
	...
	span := e.createInvokeStepSpan(ctx, i, gen, evt, pause, &nextItem, lifecycleItem, now, handledAt)
...
func (e *executor) createInvokeStepSpan(ctx context.Context, i *runInstance, gen state.GeneratorOpcode, evt event.BaseTrackedEvent, pause state.Pause, nextItem *queue.Item, lifecycleItem queue.Item, now time.Time, handledAt time.Time) *tracing.DroppableSpan {
	attrs := tracing.GeneratorAttrs(&gen)
	// The invoke is queued when the opcode is handled, not when the reporting
	// request was enqueued: under checkpointing that request predates sibling
	// steps run within it, and the trace UI sorts run children by queuedAt.
	tracing.AddTimingAttrs(attrs, handledAt, handledAt, time.Time{}, time.Time{})
