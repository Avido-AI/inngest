	...
	err = util.Crit(ctx, "consume pause", func(ctx context.Context) error {
		...
		if shouldEnqueueDiscovery(consumeResult.HasPendingSteps, pause.ParallelMode) {
			...
			if itemAlreadyExists {
				if pause.ParallelCoalesceKey != "" && pause.ParallelMode != enums.ParallelModeRace {
					e.log.Debug(
						"discovery enqueue after pause resume deduped via coalesce key",
						"run_id", md.ID.RunID.String(),
						"job_id", jobID,
						"coalesce_key", pause.ParallelCoalesceKey,
...
func (e *executor) buildSingleInvokeItem(ctx context.Context, i *runInstance, input batchInvokeInput, edge queue.PayloadEdge, eventName string, lifecycleItem queue.Item, now time.Time, group OpcodeGroup) (batchInvokeItem, error) {
	...
	pause := e.buildInvokePause(i, gen, edge, groupID, pauseID, opcode, eventName, strExpr, correlationID, evt, opts, carrier, expires, now, group)
