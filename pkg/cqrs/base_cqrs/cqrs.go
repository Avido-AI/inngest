...
func (w wrapper) getTraceRunsCount(ctx context.Context, opt cqrs.GetTraceRunOpt) (int, error) {
	...
	var count int64
	err = w.adapter.Conn().QueryRowContext(ctx, query, args...).Scan(&count)
	if err != nil {
		return 0, err
	}
...
func (w wrapper) getSpanRunsCount(ctx context.Context, opt cqrs.GetTraceRunOpt) (int, error) {
	...
	if opt.Filter.CEL != "" {
		expHandler, err := run.NewExpressionHandler(ctx,
			run.WithExpressionHandlerBlob(opt.Filter.CEL, "\n"),
			run.WithExpressionSQLConverter(helpers.CELConverter()),
		)
		if err != nil {
			return 0, err
		}
	...
	q := sq.Dialect(helpers.GoquDialect()).From("spans")
	if useJoin {
		q = helpers.BuildEventJoin(q)
	}

	q = q.Select(sq.L("COUNT(DISTINCT spans.run_id)").As("count")).
		Where(sq.L("spans.dynamic_span_id").In(
			sq.Dialect(helpers.GoquDialect()).Select("dynamic_span_id").Distinct().From("spans").Where(sq.C("name").Eq(meta.SpanNameRun)),
		)).
		Where(allFilters...)
