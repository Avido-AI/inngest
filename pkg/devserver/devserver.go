...
func start(ctx context.Context, opts StartOpts) error {
	...
	// Apply Postgres connection pool settings to the shared *sql.DB when using postgres.
	if opts.PostgresURI != "" || azure.IsAzureAuthEnabled() {
		if opts.PostgresMaxIdleConns > 0 {
			db.SetMaxIdleConns(opts.PostgresMaxIdleConns)
		}
		if opts.PostgresMaxOpenConns > 0 {
			db.SetMaxOpenConns(opts.PostgresMaxOpenConns)
		}
		if opts.PostgresConnMaxIdleTime > 0 {
			db.SetConnMaxIdleTime(time.Duration(opts.PostgresConnMaxIdleTime) * time.Second)
		}
		if opts.PostgresConnMaxLifetime > 0 {
			db.SetConnMaxLifetime(time.Duration(opts.PostgresConnMaxLifetime) * time.Second)
		}
	}
