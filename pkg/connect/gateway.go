...
func (c *connectGatewaySvc) Handler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			...
			if phase := ch.phase(); workerClosed || phase >= gatewayConnPhaseDisconnecting {
				ch.log.Debug("skipping draining status update because the connection is already disconnecting",
					"phase", phase.String(),
